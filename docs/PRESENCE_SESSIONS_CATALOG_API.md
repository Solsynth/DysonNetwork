...
- **Catalog** — a persistent entity per `(account, provider, catalog_key)` holding names, metadata, category, visibility, and the accumulated `total_seconds` / `session_count`. Clients can answer "how long did I spend in Project Omega this week".
- **Stats** — a per-catalog seconds summary computed from ended session rows, filterable by time range and category — the "how long per thing" dashboard endpoint.
- **Strong filtering** — the existing history endpoint now also filters by `category`, `visibility`, `catalog_id`, and session start-time range.
...
| Category | High-level classification: `Unknown, Gaming, Coding, Music, Productivity, Entertainment, Social, Fitness, Reading, Other` |
| Visibility | `Unknown, Public, Friends, Private` |

### Enum ordinals

`PresenceCategory` and `PresenceVisibility` are separate from the existing `PresenceType` (`Unknown, Gaming, Music, Workout`). Clients send both `type` (wire type) and `category` (classification) explicitly; there is no automatic mapping.
...
|---|---|---|---|
| `type` | string (enum) | required | `Unknown`, `Gaming`, `Music`, `Workout` |
| `manual_id` | string | required | Stable session identifier; re-starting refreshes instead of duplicating |
| `category` | string (enum) | `Unknown` | High-level classification |
...
|---|---|---|---|
| `category` | string (enum) | — | Filter by catalog category |
...
| `category` | string (enum) | — | Only count catalog items of this category |
...
| Parameter | Type | Default | Description |
|---|---|---|---|
| `category` | string (enum) | — | Filter by activity category |
...
{ "manual_id": "vscode", "type": "Gaming", "category": "Coding",
...
public class SnPresenceActivity : ModelBase
{
    ...
    public PresenceCategory Category { get; set; }          // new

...
""";
    ...
    )
    {
        ...
        var expandedUserEvents = ExpandRecurringEventsForCountdown(userEvents, now, includePast: true);
        ...
        if (includeNotableDays && !string.IsNullOrWhiteSpace(regionCode) && notableDaysService != null)
        {
            ...
            var holidays = await notableDaysService.GetNotableDays(currentYear - 1, regionCode);
            holidays.AddRange(await notableDaysService.GetNotableDays(currentYear, regionCode));
        ...
        // Combine and sort all events (ongoing first, then by distance from now)
        var allEvents = expandedUserEvents
            ...
            .ThenBy(e => Math.Abs((e.StartTime - now).TotalMinutes))
            .ToList();
    ...
    )
    {
        ...
        foreach (var evt in events)
        {
            ...
            // Expand recurring events (more occurrences if including past)
            var maxOccurrences = includePast ? 20 : 10;
            var occurrences = GetRecurringEventOccurrencesForCountdown(evt, fromTime, maxOccurrences, includePast);
            expandedEvents.AddRange(occurrences);
        }
    ...
    )
    {
        ...
        // If including past events, collect past occurrences first
        if (includePast)
        {
            ...
            // Go back to find some past occurrences
            while (occurrenceCount < maxOccurrencesLimit / 2 && tempEnd <= fromTime)
            {
            ...
            // Keep only the last few past occurrences
            if (pastOccurrences.Count > 5)
                pastOccurrences = pastOccurrences.Skip(pastOccurrences.Count - 5).ToList();

            occurrences.AddRange(pastOccurrences);
        ...
        {
            if (currentEnd > fromTime || includePast)
            {

...
""";
    ...
    )
    {
        ...
        // If including past events, collect the last past occurrence
        if (includePast)
        {
            ...
            while (occurrenceCount < maxOccurrencesLimit && tempEnd <= fromTime)
            {
                pastOccurrences.Add(CreateOccurrence(evt, tempStart, tempEnd));
                occurrenceCount++;
                (tempStart, tempEnd) = CalculateNextOccurrence(tempStart, tempEnd, recurrence);
                if (tempStart == default)
                    break;
                if (endDate.HasValue && tempStart > endDate.Value)
                    break;
            }

            // Keep only the last past occurrence
            if (pastOccurrences.Count > 0)
                occurrences.Add(pastOccurrences[^1]);
        ...
        // Generate future occurrences, filling up to maxOccurrences total
        occurrenceCount = 0;
        var remainingSlots = maxOccurrences - occurrences.Count;
        while (
            occurrenceCount < remainingSlots
            && (!endDate.HasValue || currentStart <= endDate.Value)
        )
        {
            if (currentEnd > fromTime)
            {
                occurrences.Add(CreateOccurrence(evt, currentStart, currentEnd));
            }

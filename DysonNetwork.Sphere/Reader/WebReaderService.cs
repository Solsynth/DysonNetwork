...
)
{
    ...
    )
    {
        ...
        try
        {
            ...
            {
                logger.LogWarning(
                    "URL is not an HTML page: {Url}, ContentType: {ContentType}",
                    url,
                    contentType
                );
    ...
    private async Task<LinkEmbed> ExtractLinkData(string url, string html, Uri uri)
    {
        ...
        var metaTitle = GetMetaTagContent(document, "title") ?? GetMetaContent(document, "title");
        ...
        var publishedTime =
            GetMetaTagContent(document, "article:published_time")
            ?? GetMetaTagContent(document, "datePublished")
            ?? GetMetaTagContent(document, "pubdate");

        var author =
            GetMetaTagContent(document, "author")
        ...
        embed.SiteName =
            ogSiteName ?? jsonLd.SiteName ?? GetMetaContent(document, "application-name") ?? uri.Host;
        embed.ContentType = ogType ?? jsonLd.Type;
        embed.FaviconUrl = faviconUrl;
        embed.Author = author ?? jsonLd.Author;

        if (
            !string.IsNullOrEmpty(publishedTime)
            && DateTime.TryParse(
                publishedTime,
            ...
            try
            {
                ...
                foreach (var element in elements)
                {
                    if (string.IsNullOrEmpty(result.Title))
                        result.Title =
                            GetStringProperty(element, "headline")
                    ...
                    {
                        var authorProp =
                            element.TryGetProperty("author", out var authorEl) ? authorEl : default;
                        if (authorProp.ValueKind == System.Text.Json.JsonValueKind.Object)
                            ...
                            result.Author = authorProp.EnumerateArray().FirstOrDefault()
                                .TryGetProperty("name", out var nameEl)
                                ? nameEl.GetString()
                                : null;
                    }
    ...
    private static System.Text.Json.JsonElement? GetNestedProperty(
    ...
    private async Task CacheLinkPreview(LinkEmbed? linkEmbed, string url, TimeSpan? expiry = null)
    {
        ...
        try
        {
            ...
            await cache.SetWithGroupsAsync(cacheKey, linkEmbed, [LinkPreviewCacheGroup], expiryTime);

            logger.LogDebug(
                "Cached link preview for URL: {Url} with key: {CacheKey}",
                url,
                cacheKey
            );
        }
